...
export default function ArchiveScreen() {
  ...
  const [selectedYear, setSelectedYear] = useState(route.params?.year?.toString() || '');
  const [selectedCategory, setSelectedCategory] = useState(route.params?.category_id?.toString() || '');
  const [selectedAuthor, setSelectedAuthor] = useState(route.params?.author_id?.toString() || '');

  // Load Dropdown Data
  useEffect(() => {
    const fetchFilters = async () => {
      try {
        const [y, c, a] = await Promise.all([getYears(), getMenu(), getAuthor()]);
        setYears(y.data || []);
        setCategories(c || []);
        setAuthors(a.data || []);
          ...
          {isSearchMode ? (
            ...
            <View style={styles.searchBarWrapper}>
              <TextInput
                ...
                placeholder="Search..."
                onSubmitEditing={() => navigation.setParams({ search: localSearch, page: 1 })}
              />
            ...
            <View style={styles.dropdownContainer}>
              ...
              <View style={styles.pickerBorder}>
                ...
                >
                  ...
                  {authors.map(a => <Picker.Item key={a.id} label={a.name} value={a.id.toString()} />)}
                </Picker>
              </View>

              <TouchableOpacity style={styles.searchBtn} onPress={handleApplyFilters}>
